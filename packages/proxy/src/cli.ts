...
function configBackendEnvNames(
  ...
  if (kind === "nwc") {
    return {
      connectionString: existing?.connectionString ?? defaults.connectionString,
    };
  }
...
function requiredEnvEntries(
  ...
  if (config.backend.kind === "nwc") return [["connectionString", vars.connectionString]];
  return [
